...
import { z } from "zod";
import { insertMileageLogSchema, rawInsertMileageLogSchema } from "../../shared/schema.js";
import { upload } from "../middleware/multer-config.js"; // Add .js extension
import { v4 as uuidv4 } from "uuid"; // Import uuid
import { loadConfig } from "../config.js"; // Add .js extension
import { processOdometerImageWithAI } from "../util/ocr.js"; // Add .js extension
export function createMileageLogRouter(storage) {
    ...
    router.put("/:id", async (req, res, next) => {
        try {
            ...
            if (existingLog.userId !== internalUserId)
            ...
            if (validatedData.startImageUrl === null && existingLog.startImageUrl) {
                ...
                await storage.deleteFile(existingLog.startImageUrl).catch((e) => console.error(`Failed to delete old Supabase start image:`, e)); // Add type to catch
            }
            if (validatedData.endImageUrl === null && existingLog.endImageUrl) {
                ...
                await storage.deleteFile(existingLog.endImageUrl).catch((e) => console.error(`Failed to delete old Supabase end image:`, e)); // Add type to catch
            }
    ...
    router.delete("/:id", async (req, res, next) => {
        try {
            ...
            const deleteSupabaseImage = async (imagePath) => {
                if (!imagePath)
                    return;
                console.log(`Deleting image ${imagePath} from Supabase for mileage log ${logId}.`);
                await storage.deleteFile(imagePath).catch((e) => console.error(`Error deleting Supabase image ${imagePath}:`, e)); // Add type to catch
            };
    ...
    router.post("/upload-odometer-image", upload.single("odometerImage"), async (req, res, next) => {
        try {
            // Cast the request to our authenticated request type
            const authReq = req;
            const userProfile = authReq.user;
            ...
            const method = (config.defaultOcrMethod || "gemini");
            ...
            if (ocrResult.success) {
                ...
                await storage.deleteFile(supabasePath).catch((e) => console.error("Failed to delete Supabase file after OCR error:", e)); // Add type to catch
